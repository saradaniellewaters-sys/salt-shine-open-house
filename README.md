...
This version adds editable fields for:

- Your text number
- Your booking link
- The text message used by the Property Info page button

Use `{bookingLink}`, `{address}`, and `{price}` inside the text message if you want the app to automatically fill those values. Example:

`Hi Sara! I’d like to schedule a showing for {address}. Here is your booking link: {bookingLink}`

The QR generator now uses shorter encoded keys and a fallback QR image if the primary QR library does not load. After updating the files on Netlify/GitHub Pages, regenerate and re-download your QR codes. Old QR downloads will still point to the old version.
